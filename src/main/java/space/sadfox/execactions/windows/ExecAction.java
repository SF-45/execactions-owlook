...
public class ExecAction implements Action {
	...
	public ExecAction(ActionEntity actionEntity, TableData target, ExecCommand provider) {
		...
		// -----------------Mode-------------------
		StringProperty execModeProp = actionEntity.getActionProperty(Properties.EXEC_MODE.name(),
				Mode.SINGLE.name());
		...
		// -----------------Shell-------------------
		StringProperty shellProp = actionEntity.getActionProperty(Properties.EXEC_SHELL.name(),
				Shell.CMD.name());
	...
	public Controller getConfigController() {
		try {
			...
			if (mode.get().equals(Mode.SINGLE)) editNode.singleExec.setSelected(true);
			...
			if (shell.get().equals(Shell.CMD)) editNode.cmdShell.setSelected(true);
